...
class Servers::ConsolidateRankingsTest < ActiveSupport::TestCase
  ...
  describe "#call when processing current" do
    it "consolidates rankings for the current year, month" do
      env_with_values("PERIODS_MIN_PAST_TIME" => "2023-01-01T00:00:00Z") do
        ...
        create_server_vote(server: server1, game:, created_at: "2023-12-31T23:59:59Z") # Server1, NOT current year, NOT current month
        create_server_vote(server: server1, game:, created_at: "2024-01-01T00:00:00Z") # Server1, current year, NOT current month
        create_server_vote(server: server1, game:, created_at: "2024-03-01T12:00:00Z") # Server1, current year, NOT current month
        create_server_vote(server: server1, game:, created_at: "2024-04-01T12:00:00Z") # Server1, current year, NOT current month
        create_server_vote(server: server1, game:, created_at: "2024-09-01T23:59:59Z") # Server1, current year, current month
        create_server_vote(server: server1, game:, created_at: "2024-09-02T00:00:00Z") # Server1, current year, current month
        create_server_vote(server: server1, game:, created_at: "2024-09-06T12:00:00Z") # Server1, current year, current month
        create_server_vote(server: server1, game:, created_at: "2024-09-07T12:00:00Z") # Server1, current year, current month
        create_server_vote(server: server1, game:, created_at: "2024-09-08T12:00:00Z") # Server1, current year, current month

        create_server_vote(server: server2, game:, created_at: "2024-01-01T00:00:00Z") # Server2, current year, NOT current month
        create_server_vote(server: server2, game:, created_at: "2024-01-02T00:00:00Z") # Server2, current year, NOT current month
        create_server_vote(server: server2, game:, created_at: "2024-01-03T00:00:00Z") # Server2, current year, NOT current month
        create_server_vote(server: server2, game:, created_at: "2024-01-04T00:00:00Z") # Server2, current year, NOT current month

        create_server_vote(server: server3, game:, created_at: "2024-09-01T12:00:00Z") # Server3, current year, current month
        create_server_vote(server: server3, game:, created_at: "2024-09-01T13:00:00Z") # Server3, current year, current month
        create_server_vote(server: server3, game:, created_at: "2024-09-01T14:00:00Z") # Server3, current year, current month
        create_server_vote(server: server3, game:, created_at: "2024-09-01T15:00:00Z") # Server3, current year, current month
        create_server_vote(server: server3, game:, created_at: "2024-09-01T23:59:59Z") # Server3, current year, current month
        ...
        travel_to(current_time) do
          assert_difference(-> { ServerStat.count }, 5) do
          ...
          assert_equal(5, ServerStat.where(ranking_number_consolidated_at: current_time).count)
        ...
        travel_to(current_time + 1.hour) do
          ...
          assert_equal(5, ServerStat.where(ranking_number_consolidated_at: current_time + 1.hour).count)
          server_stat_assertions.call
        end
      end
    end
  ...
  describe "#call when processing all" do
    it "consolidates rankings for all years, months" do
      env_with_values("PERIODS_MIN_PAST_TIME" => "2023-01-01T00:00:00Z") do
        current_time = Time.iso8601("2024-09-08T18:00:00Z")
        game = create_game
        server1 = create_server
        ...
        travel_to(current_time) do
          assert_difference(-> { ServerStat.count }, 11) do
          ...
          assert_equal(11, ServerStat.where(ranking_number_consolidated_at: current_time).count)
          server_stat_assertions.call
        end
        ...
        travel_to(current_time + 1.hour) do
          ...
          assert_equal(11, ServerStat.where(ranking_number_consolidated_at: current_time + 1.hour).count)
          server_stat_assertions.call
        end
      end
    end
