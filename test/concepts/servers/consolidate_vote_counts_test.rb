...
class Servers::ConsolidateVoteCountsTest < ActiveSupport::TestCase
  ...
  describe "#call when processing current" do
    it "consolidates vote counts for the current year, month" do
      env_with_values("PERIODS_MIN_PAST_TIME" => "2023-01-01T00:00:00Z") do
        ...
        create_server_vote(server:, game: game1, created_at: "2023-12-31T23:59:59Z") # Game1, NOT current year, NOT current month
        create_server_vote(server:, game: game1, created_at: "2024-01-01T00:00:00Z") # Game1, current year, NOT current month
        create_server_vote(server:, game: game1, created_at: "2024-03-01T12:00:00Z") # Game1, current year, NOT current month
        create_server_vote(server:, game: game1, created_at: "2024-09-01T23:59:59Z") # Game1, current year, NOT current month
        create_server_vote(server:, game: game1, created_at: "2024-09-02T00:00:00Z") # Game1, current year, current month
        create_server_vote(server:, game: game1, created_at: "2024-09-06T12:00:00Z") # Game1, current year, current month

        create_server_vote(server:, game: game1, created_at: "2024-09-07T12:00:00Z") # Game1, current year, current month
        create_server_vote(server:, game: game1, created_at: "2024-09-08T12:00:00Z") # Game1, current year, current month

        create_server_vote(server:, game: game2, created_at: "2024-09-08T15:00:00Z") # Game2, current year, current month
        create_server_vote(server:, game: game2, created_at: "2024-09-08T18:00:00Z") # Game2, current year, current month
        ...
        travel_to(current_time) do
          assert_difference(-> { ServerStat.count }, 4) do
            described_class.call(server)
          end

          assert_equal(4, ServerStat.where(vote_count_consolidated_at: current_time).count)
        ...
        travel_to(current_time + 1.hour) do
          ...
          assert_equal(4, ServerStat.where(vote_count_consolidated_at: current_time + 1.hour).count)
          server_stat_assertions.call
        end
      end
    end
  ...
  describe "#process_all" do
    it "consolidates vote counts for all years, months" do
      env_with_values("PERIODS_MIN_PAST_TIME" => "2023-01-01T00:00:00Z") do
        current_time = Time.iso8601("2024-09-08T18:00:00Z")
        game1 = create_game
        game2 = create_game
        ...
        travel_to(current_time) do
          assert_difference(-> { ServerStat.count }, 8) do
            described_class.call(server, nil, Periods.min_past_time, current_time)
          end

          assert_equal(8, ServerStat.where(vote_count_consolidated_at: current_time).count)
          server_stat_assertions.call
        end
        ...
        travel_to(current_time + 1.hour) do
          ...
          assert_equal(8, ServerStat.where(vote_count_consolidated_at: current_time + 1.hour).count)
          server_stat_assertions.call
        end
      end
    end
